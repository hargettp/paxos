...
leadBasicPaxosBallot p d = do
  ...
  get >>= \ledger -> do
    let maybeChosenDecree = chooseDecree (paxosMembers ledger) d earlierVotes
...
set ledger = Paxos $ \_ -> return (ledger,())

get :: Paxos d (Ledger d)
get = Paxos $ \ledger -> return (ledger,ledger)
